...
while(nDay <= fDay) {
  ...
  if(animal == 'dairy') {
    # nHeifer_first_lact = nHeifer.first.dry+nHeifer.first.lact
    # nHeifer_second_lact = nHeifer.second.dry+nHeifer.second.lact
    # nHeifer_third_lact = nHeifer.third.dry+nHeifer.third.lact
    
    # born <- ((kFreshening*nDry*0.5)+(kFreshening*nHeifer.first.dry*0.5)+
    #            +(kMature*nCalf*0.5)+
    #   (kFreshening*nHeifer.second.dry*0.5)+(kFreshening*nHeifer.third.dry*0.5))*(1-kMortality)/100

    nCalf <- target + born
    born <- (target*0.5)/365
    calves_death <- nCalf*kMortality/365
    calves_cull <- nCalf*kHeiferCull/365
    age_out <- (nCalf )*(1/(365*2))-calves_death*0.5-calves_cull*0.5
    ...
    nHeifer_fourth_lact <- third_age_out*365
    fourth_lact_death <- nHeifer_fourth_lact*kMortality/365
    fourth_lact_cull <- nHeifer_fourth_lact*fourthCull/365
    fourth_age_out <- (nHeifer_fourth_lact/365)-fourth_lact_death-fourth_lact_cull

    
    nLact <- nHeifer_first_lact+nHeifer_second_lact+nHeifer_third_lact+nHeifer_fourth_lact
    # nHeifer.second.dry <- nHeifer.second.dry  + (first_age_out - second_lact_cull - second_lact_death)*(60/365)
    # nHeifer.third.dry <- nHeifer.third.dry + (second_age_out - third_lact_cull - third_lact_death)*(60/365)
    # nHeifer.first.lact <- nHeifer.first.lact + (age_out - first_lact_cull - first_lact_death)*(305/365)
    # nHeifer.second.lact <- nHeifer.second.lact + (first_age_out - second_lact_cull - second_lact_death)*(305/365)
    # nHeifer.third.lact <- nHeifer.third.lact + (second_age_out - third_lact_cull - third_lact_death)*(305/365)
    # 
    
    # new.lact.herd <- (kFreshening*nDry)*kMortality
    # new.first.lact <- kMature*nCalf*kMortality
  ...
  # val[8,nDay] <- nDry
  val[9,nDay] <- nCalf
  # val[10,nDay] <- nHeifer.first.lact+nHeifer.second.lact+nHeifer.third.lact

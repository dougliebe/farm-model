...
!      added vars
     ...
     do   nDay = 1, fDay
	  ...
	  cumManure = (intake- DIGIntake)*(nChicks+ nBroilers)
      ! Calc N and P
      ! Calc N and P
      Nexc = brlNex()
	  Nmin = (0.088*Nexc)
     ...
     &       ADG*nChicks)
      ...
      !update animal numbers
       !update animal numbers
      if (nBroilers .gt. 0) nBroilers = nBroilers-&
	  &(kMortality*iChicks/(switch_feed*2)/100)
      if (nChicks .gt. 0) nChicks = nChicks-&
	  &(kMortality*iChicks/(switch_feed*2)/100)
      !wt updating
      meat_produced = culling*(wtBroilers)
      
      if (mod(nDay,(switch_feed*2)) .eq. 0) then
            manure_out = cumManure + manure_out
	  ...
	  !if (nDay > switch_feed*2) nDay = 1
      !print *,nDay, Porg,Nmin,Pmin,cumManure,manureStore
        print *,nDay,manureStore,manure_out,cumNorg,cumNmin,cumPorg,cumPmin,cumManure,nBroilers,nChicks,wtBroilers,wtChicks
      end do
     ! print *,nDay, Porg,Nmin,Pmin,cumManure,manureStore
       !!nDay = nDay + 1
...
!             if (nChicks .gt. 0) wtChicks = (wtChicks + ADG*nChicks)
     ...
     &         - (2.0772E-3*Temp*(wt_per_bird*1000.0)))
      ...
      function brlDMI(ADGg) result(DMI)
            implicit none
            real, intent(in) :: ADGg
            real :: DMI, intake_ME, fed_ME,wt_per_bird 
			wt_per_bird =(wtChicks+wtBroilers)/(nChicks+nBroilers)

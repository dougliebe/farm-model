...
if(animal == 'layer') {
  ...
  # Cumulative BW
  total_BW <- function(days){
    week = seq(1,max(days/7,1),1)
  if(iPullets.brown > 0) {
    Wm = 2#1.9143
    b = 0.14517
    t.star = 8.57678
...
### Daily Gain
  ...
  layer_ADG <- function(day_since_new_pullets) {
    ...
    week <- round(day_since_new_pullets/7)
  ...
  egg_weight <- function(week) {
    return(ifelse(week<25,0,0.0002*week^3 - 0.0405*week^2 + 2.1377*week + 20.109))
  }

### Total N needed from SID Lys requirement
  Total.N.req <- function() {
    W <- (total_BW(day_since_new_pullets))^0.75
    ADG <- layer_ADG()
    Egg.Mass <- egg_weight(day_since_new_pullets/7)
    SID.Lys <- 0.07*W + 0.02*ADG + 0.0124*Egg.Mass
    ME.int <- (196*(total_BW(day_since_new_pullets))^0.7543)+2.4833*ADG*1000 #Convert to g
...
### Dry Matter Intake
  ...
  #Zuidhof et al 2014
  layer_Intake <- function() {
    ADG = layer_ADG()
    I_ME <- (196*(total_BW(day_since_new_pullets))^0.7543)+2.4833*ADG*1000 #Convert to g
    ME <- (113*total_BW(day_since_new_pullets)^0.75)+(6.68*layer_ADG())+2.4*egg_weight(day_since_new_pullets/7)
    Intake <- I_ME/ME
    return(Intake)
  }
...
### Excretions
  ...
  layer_N_excretion <- function(ADG) {
    nChicks = 0
    CP <- (iPullets.brown > 0)*CP.brown+(iPullets.white > 0)*CP.white
    FI <- layer_Intake()
    # For Mature Birds
    # if((nLaying.hens.brown+nLaying.hens.white+nMolt.hens.brown+nMolt.hens.white)>0) {
      Nintake <- FI*1000 * (nLaying.hens.brown+nLaying.hens.white+nMolt.hens.brown+nMolt.hens.white) * (CP/100)/6.25
      Nret <- Total.N.req()*(nLaying.hens.brown+nLaying.hens.white+nMolt.hens.brown+nMolt.hens.white)
      Nexc <- Nintake - Nret
      return(Nexc/1000) # Returns N in kg
    # }
    # # For young birds
    # else {
    #   Nintake <- FI * (nPullets.brown+nPullets.white) * (CP/100)/6.25 #N in kg
  ...
  layer_P_excretion <- function(ADG) {
    ...
    Pexc <- FI*P_exc*sum(nlayers, (nPullets.brown+nPullets.white))

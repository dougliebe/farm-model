...
integrate.milk <- function(days, Fat, Protein, cECM) {
  PKYDa = (0.125*5+0.375)*PKYD
  A = 1/(PKYDa*(1/8.5)*exp(1))
  my = sum(((days%%365/7)*(days%%365 <= 305))/(A*exp((1/8.5)*(days%%365/7))))*
...
FMY <- function(df) {
  ...
  total.milk = sum((seq(1,305,1)%%365/7)/(A*exp((1/8.5)*(seq(1,305,1)%%365/7))))*df$cECM
  return(currentlact+second+third+fourth)
}
